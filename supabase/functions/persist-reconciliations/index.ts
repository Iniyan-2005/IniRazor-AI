...
serve(async (req) => {
  ...
  try {
    ...
    // Prepare records: explicitly inject the user's ID into every row
    const recordsToUpsert = reconciliations
      .filter((recon: any) => !recon.payment_id.startsWith('pay_demo_'))
      .map((recon: any) => ({
        id: recon.id,
